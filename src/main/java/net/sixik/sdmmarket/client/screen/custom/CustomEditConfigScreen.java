package net.sixik.sdmmarket.client.screen.custom;
...
public class CustomEditConfigScreen extends AbstractThreePanelScreen<CustomEditConfigScreen.ConfigPanel> {
    private ConfigGroup group;
    private Component title;
    private List<Widget> allConfigButtons; // both groups and entries
    ...
    private MarketEntry<?> marketEntry;

    public CustomEditConfigScreen(MarketEntry<?> marketEntry) {
        super();
    ...
    public void updateGroup(){
        ...
        ConfigGroup group1 = new ConfigGroup("entry", groupButton -> {
            openGui();
        ...
        marketEntry.getConfig(group1);

        this.group = group1;
        title = group1.getName().copy().withStyle(ChatFormatting.BOLD);
     ...
     */
    public CustomEditConfigScreen setAutoclose(boolean autoclose) {
        ...
        public void onClicked(MouseButton button) {
            if (getMouseY() >= 20) {
                ...
                configValue.onClicked(this, button, accepted -> {
                    if (accepted) {
                        CustomEditConfigScreen.this.updateGroup();
                        changed = true;
                    }
                    run();
                });
    ...
    public class ConfigPanel extends Panel {
        public ConfigPanel() {
            super(CustomEditConfigScreen.this);
        }
